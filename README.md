...
In the file `main.cpp`, the config file is first parsed. Based on the user arguments, a factory class `MatrixGeneratorFactory` will instantiate the correct child class, derived from the abstract `MatrixGenerator`class. The `GenerateMatrix()` method of the child of this class returns a pointer to matrix $A$ in the main flow. This part is managed by the function `CreateMatrix()`.

Then, based on user input, the class `SolverFactory` instantiates the desired solver class, which is a child of the abstract class `AbstractIterativeSolver`, with either specified or default method arguments (_tolerance_, _maxIter_ and _shift_). Subsequently, the eigenvalues are generated by calling the solver's `FindEigenvalues()` method. This part is managed by the function `SolveProblem()`
