...
std::unique_ptr<AbstractIterativeSolver<T>> SolverFactory<T>::ChooseSolver()
{
    ...
    if (methodArgs.size() > 0)
    {
        ...
        {
            std::cerr << "[WARNING] Error processing the argument 'tolerance': " << e.what() << std::endl
    ...
    if (methodArgs.size() > 1)
    {
        ...
        {
            std::cerr << "[WARNING] Error processing the argument 'maxIter': " << e.what() << std::endl
    ...
    if (methodArgs.size() > 2)
    {
        ...
        {
            std::cerr << "[WARNING] Error processing the argument 'shift': " << e.what() << std::endl
