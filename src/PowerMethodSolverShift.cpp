...
template <typename T>
void PowerMethodSolverShift<T>::SetInitialGuess(const Eigen::Vector<T, Eigen::Dynamic> x_0){ initialGuess=x_O; }
...
void PowerMethodSolverShift<T>::FindEigenvalues(std::ostream &stream) {
    ...
    A_shifted = A - mu *  MatrixXd::Identity(n_rows, n_columns);

    while (error > tolerance && iter_count < max_iter) {
        // Multiply A * x_0
        Eigen::VectorXf Ax = A_shifted * x_ini;
